...
export function ChatHeader({ setMobileView }: ChatHeaderProps) {
  return (
    <header className="flex h-17 items-center gap-3 border-b border-border bg-card px-4 shadow-sm sm:px-7">
      <button onClick={() => setMobileView('sidebar')} aria-label="Back to chats" className="rounded-full p-2 hover:bg-accent lg:hidden">
        <ArrowLeft className="w-5 h-5" />
      </button>
      <span className="grid size-10 place-items-center rounded-full bg-sky-500 text-sm font-semibold text-white">QL</span>
